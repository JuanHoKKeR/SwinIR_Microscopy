...
def main():
    ...
    # Modificado para aceptar archivos individuales
    parser.add_argument('--input_path', type=str, default=None, help='input low-quality test image file')
    parser.add_argument('--gt_path', type=str, default=None, help='input ground-truth test image file (optional)')
    parser.add_argument('--output_path', type=str, default=None, help='output path for result (optional)')
    
    # Mantener compatibilidad con carpetas
    ...
    # Determinar si usar archivos individuales o carpetas
    if args.input_path:
        # Modo de archivo individual
        process_single_image(args, model, device)
    else:
        # Modo de carpeta (código original)
        process_folder(args, model, device)
...
def process_single_image(args, model, device):
    ...
    if not os.path.exists(args.input_path):
        print(f"Error: No se encontró el archivo de entrada: {args.input_path}")
        return
    
    # setup folder and path
    border, window_size = setup_single(args)
    ...
    if args.output_path:
        ...
        # Crear nombre de salida automático
        input_dir = os.path.dirname(args.input_path)
        input_name = os.path.splitext(os.path.basename(args.input_path))[0]
        save_path = os.path.join(input_dir, f"{input_name}_SwinIR.png")

    # read image
    imgname, img_lq, img_gt = get_image_pair_single(args)
    ...
    # evaluate psnr/ssim/psnr_b if ground truth is available
    if img_gt is not None:
        ...
        if img_gt.ndim == 3:
            ...
            print(f'SSIM_Y: {ssim_y:.4f}')
        if args.task in ['jpeg_car']:
            print(f'PSNR_B: {psnr_b:.2f} dB')
    else:
        print('Testing completado sin ground truth para evaluación')


def process_folder(args, model, device):
    """Procesa carpeta completa (código original)"""
    import glob
...
def get_image_pair_single(args):
    """Carga imagen individual y opcionalmente ground truth"""
    input_path = args.input_path
    gt_path = args.gt_path
    
    (imgname, imgext) = os.path.splitext(os.path.basename(input_path))

    # Cargar imagen de entrada
    if args.task in ['classical_sr', 'lightweight_sr', 'real_sr', 'color_dn']:
        img_lq = cv2.imread(input_path, cv2.IMREAD_COLOR)
        if img_lq is None:
            return imgname, None, None
        img_lq = img_lq.astype(np.float32) / 255.
        
    elif args.task in ['gray_dn', 'jpeg_car']:
        img_lq = cv2.imread(input_path, cv2.IMREAD_GRAYSCALE)
        if img_lq is None:
            return imgname, None, None
        img_lq = img_lq.astype(np.float32) / 255.
    ...
    if gt_path and os.path.exists(gt_path):
        if args.task in ['classical_sr', 'lightweight_sr', 'color_dn']:
    ...
    # Para tareas de denoising, generar ruido sintético si no hay GT
    if args.task in ['gray_dn', 'color_dn'] and img_gt is None:
        # Usar la imagen de entrada como GT y generar versión con ruido
        img_gt = img_lq.copy()
        np.random.seed(seed=0)
        img_lq = img_gt + np.random.normal(0, args.noise / 255., img_gt.shape)
    
    # Para JPEG, generar compresión sintética si no hay GT
    elif args.task in ['jpeg_car'] and img_gt is None:
        img_gt = (img_lq * 255).astype(np.uint8)
        result, encimg = cv2.imencode('.jpg', img_gt.squeeze(), [int(cv2.IMWRITE_JPEG_QUALITY), args.jpeg])
        img_lq_jpeg = cv2.imdecode(encimg, 0)
        img_gt = img_gt.astype(np.float32) / 255.
        img_lq = np.expand_dims(img_lq_jpeg.astype(np.float32) / 255., axis=2)

    return imgname, img_lq, img_gt


def setup_single(args):
    """Setup para imagen individual"""
